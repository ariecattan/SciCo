...
class SimplePairWiseClassifier(nn.Module):
    ...
    def forward(self, first, second):
        pairs = torch.cat((first, second, first * second), dim=1)
        scores = self.pairwise_mlp(pairs)
...
class EntailmentModel(nn.Module):
    def __init__(self, nli_model, device):
    ...
    def forward(self, premise, hypothesis):
        ...
        token_ids = torch.tensor(tokens['input_ids'], device=self.device)
        attention_mask = torch.tensor(tokens['attention_mask'], device=self.device)
        scores = self.model(token_ids, attention_mask).logits
        scores = torch.softmax(scores, dim=1)
        prediction = torch.argmax(scores, dim=1)

        return scores[:, -1], prediction
